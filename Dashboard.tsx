...
import { Briefcase, Scale, Clock, AlertCircle, ArrowUpRight, ArrowRight, Gavel, Calendar } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <StatCard 
          title="Next Hearing" 
          value={nextHearing ? new Date(nextHearing.nextHearing).getDate().toString() : '-'} 
          subtext={nextHearing ? nextHearing.title : 'No upcoming hearings'} 
          icon={AlertCircle} 
          color="bg-rose-500" 
        />
      </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm flex flex-col">
           <div className="p-4 border-b border-slate-100 flex items-center justify-between bg-amber-50/50 rounded-t-xl">
              <h2 className="font-semibold text-amber-900 flex items-center gap-2">
                <Calendar className="w-4 h-4" />
                Tomorrow's Listings ({tomorrowStr})
              </h2>
              <span className="text-xs font-medium bg-white text-amber-600 px-2 py-1 rounded border border-amber-100">
                  {tomorrowsListings.length} Cases
              </span>
           </div>
           <div className="p-4 space-y-3">
              {tomorrowsListings.length > 0 ? (
                  tomorrowsListings.map(c => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm flex flex-col">
          ...
          <div className="p-6 space-y-4">
            ...
            {upcomingHearings.length === 0 && <p className="text-sm text-slate-500 text-center">No upcoming hearings scheduled.</p>}
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm flex flex-col">
          ...
          <div className="p-6 space-y-4">
            {highPriorityTasks.slice(0, 3).map(task => (
              <div key={task.id} className="flex items-center justify-between p-3 border border-slate-100 rounded-lg">
            ...
            {highPriorityTasks.length === 0 && <p className="text-sm text-slate-500 text-center">No high priority tasks.</p>}
